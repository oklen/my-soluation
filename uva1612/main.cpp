...
int main()
{
    int n,cnt = 1;
    while(scanf("%d",&n)&&n){
        vector<priority_queue<double,vector<double>,greater<double>>> scores(
                    n,priority_queue<double,vector<double>,greater<double>>());
        vector<double> allsum(n,0);vector<double> cc(3,0),cl;
        for(int i = 0;i<n;++i)
        {
            for(int j = 0;j<3;++j){
                double cache;scanf("%lf",&cache);
                cc[j] = cache;
                allsum[i]+=cache;
            }
            for(int j = 0;j<3;++j){
...
//            exit(0);
        ...
        lrks = rks;double mx = allsum[rks-1];
        for(int i = 1;i<n;++i){
            scanf("%d",&rks);
            if(rks>lrks){
                while((!scores[rks-1].empty())){
                    if((float)(allsum[rks-1]-scores[rks-1].top())<=(float)mx){
